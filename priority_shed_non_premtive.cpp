// Write a program C to calculate average waiting time and average 
// turn around time using Priority scheduling algorithm.
...
int main(){
    ...
    for(int i=0;i<n;++i){
       int a,b,c,d;
       // a=> prog no. // b=> arrival time // c=> burst time // d=> priority
       cin>>a>>b>>c>>d;
       vt[i].push_back(a);
       vt[i].push_back(b);
       vt[i].push_back(c);
    ...
    while(complt!=n){
      ...
      if(temp.size()==0) {
       ...
       if(temp.size()>0){
         auto it=temp[0];
         cout<<"Program "<<it[0]<<" with arrival time:- "<<it[1]<<" brust time:- "<<it[2]<<" Priority:- "<<it[3]<<endl;
